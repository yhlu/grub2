...
{
        return (grub_uint32_t)(pud_val(pud) & PTE_PFN_MASK);
}
			  ...
			  grub_uint64_t addr, grub_uint64_t end)
{
	...
	for (; addr < end; addr = next) {
		...
		if (!pmd)
			return -1;
		ident_pmd_init(info->pmd_flag, pmd, addr, next);
		set_pud(pud, __pud((grub_uint64_t)((grub_uint32_t)pmd) | _KERNPG_TABLE));
	}
...
{
        return (grub_uint32_t)(pgd_val(pgd) & PTE_PFN_MASK);
}
			      ...
			      grub_uint64_t addr, grub_uint64_t end)
{
	...
	for (; addr < end; addr = next) {
		...
		if (result)
			return result;
		set_pgd(pgd, __pgd((grub_uint64_t)((grub_uint32_t)pud) | _KERNPG_TABLE));
	}
