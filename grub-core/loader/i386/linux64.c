...
grub_linux_boot (void)
{
  ...
  *ctx.params = linux_params;
  ...
  state.cr3 = (grub_uint64_t)(grub_uint32_t)level4p;
  state.rbx = state.rsp = 0;
  state.rsi = ctx.real_mode_target;
//  state.rip = (grub_uint64_t)ctx.params->code32_start + ((grub_uint64_t)ctx.params->ext_code32_start << 32) + 0x200; /* 0x200 is offset to startup_64 */
  state.rip = prot_mode_target + 0x200; /* 0x200 is offset to startup_64 */
  return grub_relocator64_boot (relocator, state, 0, 0xc0000000);
}
		...
		int argc, char *argv[])
{
  ...
  if (lh.header != grub_cpu_to_le32_compile_time (GRUB_LINUX_MAGIC_SIGNATURE)
      || grub_le_to_cpu16 (lh.version) < 0x020c
      || !(lh.xloadflags & (1<<1))) /* XLF_CAN_BE_LOADED_ABOVE_4G */
    {
      grub_error (GRUB_ERR_BAD_OS, "too old for linux64");
      goto fail;
    }
...
//  linux_params.ext_code32_start = (grub_uint32_t) ((prot_mode_target + lh.code32_start - GRUB_LINUX_BZIMAGE_ADDR) >> 32);
  ...
  for (i = 1; i < argc; i++)
    if (grub_memcmp (argv[i], "quiet", sizeof ("quiet") - 1) == 0)
      {
